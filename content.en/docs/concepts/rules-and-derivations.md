...
[Audit and provenance](/docs/concepts/audit-and-provenance) develops the recording and export side of the rule and derivation lifecycle. The [writing rules guide](/docs/guides/writing-rules) walks through real-world rule patterns — joins, negation, rule composition — for readers approaching the same material from the practical side. The [running derivations guide](/docs/guides/running-derivations) covers the ergonomics of the evaluate-and-accept cycle in production. The [using adapters guide](/docs/guides/using-adapters) develops the choice and configuration of the three adapter engines.